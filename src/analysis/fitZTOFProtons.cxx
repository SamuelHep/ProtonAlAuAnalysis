...
Bool_t AddPointToSpectra(TH1D *yieldHisto, TF1 *yieldFit, TGraphErrors *rawSpectra,
			 TGraphErrors *deutRawSpectra, TGraphErrors *deuteronRatioGraph, 
			 TGraphErrors *pionRawSpectra,TGraphErrors *pionRatioGraph, 
			 Int_t yIndex, Int_t mTm0Index, Int_t PID, Int_t CHARGE,bool badfit, TString IMAGEDIR="");

//Canvases
TCanvas *fittingCanvas, *fitParCanvas, *spectraCanvas,*deutSpectraCanvas, *deuteronRatioCanvas, *pionSpectraCanvas, *pionRatioCanvas;
...
std::vector<std::vector<TF1 *> > tofPionOnlyFit(nRapidityBins,std::vector<TF1 *>(nmTm0Bins,(TF1 *)NULL));
...
std::vector<TGraphErrors *> pionRawSpectraTOFProPlus(nRapidityBins,(TGraphErrors *)NULL);
std::vector<TGraphErrors *> deuteronSpectraRatio(nRapidityBins,(TGraphErrors *)NULL);
std::vector<TGraphErrors *> pionSpectraRatio(nRapidityBins,(TGraphErrors *)NULL);

std::vector <TF1 * > deuteronRatioTOFFit (nRapidityBins, (TF1 *)NULL);
std::vector <TF1 * > pionRatioTOFFit (nRapidityBins, (TF1 *)NULL);
...
void fitZTOFProtons(TString ZTOFFILE, TString SPECTRAFILE, TString STARLIB, TString ZTOFGAUSFILE, Int_t RAPIDITYINDEX=-1, TString IMAGEDIR=""){
  ...
  //TH3D *protonPlusZTOF3D = (TH3D *)inFile->Get("ProtonPlus/zTOF_ProtonPlus_Cent0");
  TH3D *protonMinusZTOF3D = (TH3D *)inFile->Get("ProtonMinus/zTOF_ProtonMinus_AllCent_tofCut");
  //TH3D *protonMinusZTOF3D = (TH3D *)inFile->Get("ProtonMinus/zTOF_ProtonMinus_Cent0");
  ...
  deuteronRatioCanvas = new TCanvas("deuteronRatioCanvas","Deuteron Ratio Canvas",20,850,800,600);
  pionSpectraCanvas = new TCanvas("pion_spectraCanvas","Pion Spectra Canvas",20,850,800,600);
  pionSpectraCanvas->SetLogy();
  pionRatioCanvas = new TCanvas("pionRatioCanvas","Pion Ratio Canvas",20,850,800,600);
  ...
  for (Int_t yIndex=0; yIndex<nRapidityBins; yIndex++){
    ...
    for (Int_t mTm0Index=12; mTm0Index<nmTm0Bins; mTm0Index++){
      ...
      tofPionOnlyFit.at(yIndex).at(mTm0Index) = NULL;
	...
	pionRawSpectraTOFProPlus.at(yIndex) = new TGraphErrors();
	pionRawSpectraTOFProPlus.at(yIndex)->SetMarkerStyle(20);
	pionRawSpectraTOFProPlus.at(yIndex)->SetName(Form("pion_rawSpectraTOF_%s_yIndex%02d",
							   particleInfo->GetParticleName(PROTON,1).Data(),						
							   yIndex));

	pionSpectraRatio.at(yIndex) = new TGraphErrors();
	pionSpectraRatio.at(yIndex)->SetMarkerStyle(20);
	pionSpectraRatio.at(yIndex)->SetName(Form("pionSpectraRatioTOF_%s_yIndex%02d",
						      particleInfo->GetParticleName(PROTON,1).Data(),
						      yIndex));
	

	deuteronRatioTOFFit.at(yIndex) = new TF1(Form("deuteronRatioFit_%02d",yIndex),"[0]*exp([1]+[2]*x)",0.4,2);
	pionRatioTOFFit.at(yIndex) = new TF1(Form("pionRatioFit_%02d",yIndex),"[0]*exp([1]+[2]*x)",0.4,2);
      ...
      Bool_t success = AddPointToSpectra(tofProtonPlusHisto.at(yIndex).at(mTm0Index),
					 ...
					 rawSpectraTOFProPlus.at(yIndex),deutRawSpectraTOFProPlus.at(yIndex),deuteronSpectraRatio.at(yIndex),pionRawSpectraTOFProPlus.at(yIndex),pionSpectraRatio.at(yIndex),
	...
	tofProtonOnlyFit.at(yIndex).at(mTm0Index)->Write();
      ...
      if( tofPionOnlyFit.at(yIndex).at(mTm0Index) ){

        tofFuncFile->cd();
        tofPionOnlyFit.at(yIndex).at(mTm0Index)->Write();
    ...
    if (rawSpectraTOFProPlus.at(yIndex)->GetN() > 3 && SAVEOBJ){
      ...
      double fitMinDeut = TMath::MinElement(deuteronSpectraRatio.at(yIndex)->GetN(), deuteronSpectraRatio.at(yIndex)->GetX() );
      double fitMaxDeut = TMath::MaxElement(deuteronSpectraRatio.at(yIndex)->GetN(), deuteronSpectraRatio.at(yIndex)->GetX() );

      deuteronRatioTOFFit.at(yIndex)->SetRange(fitMinDeut,fitMaxDeut);
      deuteronSpectraRatio.at(yIndex)->Fit(deuteronRatioTOFFit.at(yIndex),"RQM");
      deuteronRatioTOFFit.at(yIndex)->Write();

      double fitMinPion = TMath::MinElement(pionSpectraRatio.at(yIndex)->GetN(), pionSpectraRatio.at(yIndex)->GetX() );
      double fitMaxPion = TMath::MaxElement(pionSpectraRatio.at(yIndex)->GetN(), pionSpectraRatio.at(yIndex)->GetX() );

      pionRatioTOFFit.at(yIndex)->SetRange(fitMinPion,fitMaxPion);
      pionSpectraRatio.at(yIndex)->Fit(pionRatioTOFFit.at(yIndex),"RQM");
      pionRatioTOFFit.at(yIndex)->Write();

      deutRawSpectraTOFProPlus.at(yIndex)->Write();
      deuteronSpectraRatio.at(yIndex)->Write();

      pionRawSpectraTOFProPlus.at(yIndex)->Write();
      pionSpectraRatio.at(yIndex)->Write();
...
Bool_t AddPointToSpectra(TH1D *yieldHisto, TF1 *yieldFit, TGraphErrors *rawSpectra,
  ...
  tofPionOnlyFit.at(yIndex).at(mTm0Index) = new TF1("","gaus(0)",-.5,.5);
  tofPionOnlyFit.at(yIndex).at(mTm0Index)->FixParameter(0,pionGaus.GetParameter(0));
  tofPionOnlyFit.at(yIndex).at(mTm0Index)->FixParameter(1,pionGaus.GetParameter(1));
  tofPionOnlyFit.at(yIndex).at(mTm0Index)->FixParameter(2,pionGaus.GetParameter(2));
  tofPionOnlyFit.at(yIndex).at(mTm0Index)->SetName(Form("PionPlusTofFit_yIndex%02d_mTm0Index%02d",yIndex,mTm0Index));
  ...
  Double_t pion_amp = pionGaus.GetParameter(0);
  Double_t pion_ampError = pionGaus.GetParError(0);
  Double_t pion_width = pionGaus.GetParameter(2);
  Double_t pion_widthError = pionGaus.GetParError(2);
  ...
  Double_t rawYield, deut_rawYield, rawYieldError, deut_rawYieldError, pion_rawYield, pion_rawYieldError;
  ...
  pion_rawYield = ( (pion_amp * pion_width * sqrt(2.0*pi)) / zBinWidth ) * normFactor;
  pion_rawYieldError = ( (pion_amp * width * sqrt(pow(pion_ampError/pion_amp,2)+pow(widthError/width,2)) *
			  sqrt(2.0*pi)) / zBinWidth) * normFactor;

  
  double deut_fracErr = deut_rawYieldError/deut_rawYield;
  double pion_fracErr = pion_rawYieldError/pion_rawYield;
  ...
  if (rawYield > 0)
    { 
      ...
      Double_t ratioPion = pion_rawYield/rawYield;
      Double_t totPion_err = sqrt(pion_fracErr*pion_fracErr + fracErr*fracErr);  
      ...
      pionRatioGraph->SetPoint(pionRatioGraph->GetN(),GetmTm0RangeCenter(mTm0Index),ratioPion);
      pionRatioGraph->SetPointError(pionRatioGraph->GetN()-1,0,totPion_err);
  ...
  pionRawSpectra->SetPoint(pionRawSpectra->GetN(),GetmTm0RangeCenter(mTm0Index),pion_rawYield);

  pionRawSpectra->SetPointError(pionRawSpectra->GetN()-1,0,pion_rawYieldError);
  ...
  pionSpectraCanvas->cd();
  pionRawSpectra->Draw("APZ");
  pionSpectraCanvas->Update();
  ...
  pionRatioCanvas->cd();
  pionRatioGraph->Draw("APZ");
  pionRatioCanvas->Update();
